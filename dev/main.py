...
class MainFrame(wx.Frame):
    def __init__(self):
        ...
        self.dish_face_svn = ''
        self.bom_list_svn = ''
        self.change_order_path = ''
        self.change_order_svn = ''
        self.test_data_path = ''
        ...
        super().__init__(None, -1, "自动生成测试报告V1.3", size=(960, 500))
        #self.SetBackgroundColour('white')
        panel = wx.Panel(self, -1)
        panel2 = wx.Panel(panel, -1,pos=(10,20),size=(300,420))
        panel3 = wx.Panel(panel, -1, pos=(320, 20), size=(300, 420))
        panel4 = wx.Panel(panel, -1, pos=(630, 20), size=(300, 420))
        ...
        #车站方案路径窗口
        self.stationDocPathText = wx.StaticText(panel2, label="车站方案路径：", pos=(10, 110))
        self.stationDocPath = wx.TextCtrl(panel2, -1, value="请选择车站方案路径", pos=(10, 130), size=(240, 25),
                                         style=wx.TE_READONLY)
        self.stationDocPathOption = wx.Button(panel2, -1, "浏览", pos=(250, 130), size=(40, 25))
        self.Bind(wx.EVT_BUTTON, self.OnClick_stationDocPath, self.stationDocPathOption)

        #SVN号窗口
        self.stationDocSvnText = wx.StaticText(panel2, label="车站方案SVN号：", pos=(10, 170))
        self.stationDocSvn = wx.TextCtrl(panel2, -1, value="", pos=(120, 170), size=(80, 25))
        self.Bind(wx.EVT_TEXT, self.EvtText_station_doc_svn, self.stationDocSvn)

        self.ioListSvnText = wx.StaticText(panel2, label="IO配线表SVN号：", pos=(10, 210))
        self.ioListDocSvn = wx.TextCtrl(panel2, -1, value="", pos=(120, 210), size=(80, 25))
        self.Bind(wx.EVT_TEXT, self.EvtText_io_list_svn, self.ioListDocSvn)

        self.dishFaceSvnText = wx.StaticText(panel2, label="盘面图SVN号：", pos=(10, 250))
        self.dishFaceSvn = wx.TextCtrl(panel2, -1, value="", pos=(120, 250), size=(80, 25))
        self.Bind(wx.EVT_TEXT, self.EvtText_dish_face_svn, self.dishFaceSvn)

        self.bomListSvnText = wx.StaticText(panel2, label="BOM表SVN号：", pos=(10, 290))
        self.bomListSvn = wx.TextCtrl(panel2, -1, value="", pos=(120, 290), size=(80, 25))
        self.Bind(wx.EVT_TEXT, self.EvtText_bom_list_svn, self.bomListSvn)

        # 变更单路径窗口
        self.changeOrderPathText = wx.StaticText(panel2, label="变更单路径：", pos=(10, 320))
        self.changeOrderPath = wx.TextCtrl(panel2, -1, value="请选择变更单路径", pos=(10, 340), size=(240, 25),
                                          style=wx.TE_READONLY)
        self.stationDocPathOption = wx.Button(panel2, -1, "浏览", pos=(250, 340), size=(40, 25))
        self.Bind(wx.EVT_BUTTON, self.EvtText_change_order_path, self.stationDocPathOption)

        # 变更单SVN号窗口
        self.changeOrderPathSvnText = wx.StaticText(panel2, label="变更单SVN号：", pos=(10, 380))
        self.changeOrderPathSvn = wx.TextCtrl(panel2, -1, value="", pos=(120, 380), size=(80, 25))
        self.Bind(wx.EVT_TEXT, self.EvtText_change_order_svn, self.changeOrderPathSvn)
        ...
        #轮数确定按钮
        self.num_ensure = wx.Button(panel3, -1, "确定", pos=(190, 350), size=(60, 40))
        self.Bind(wx.EVT_BUTTON, self.OnClick_Num_ensure, self.num_ensure)
        self.num_ensure.Enable(False)
        #轮数清空按钮
        self.num_clear = wx.Button(panel3, -1, "重置", pos=(40, 350), size=(60, 40))
        ...
        self.docNumText = wx.StaticText(panel4, label="文档编号：", pos=(10, 100))
        self.docNum = wx.TextCtrl(panel4, -1, value="", pos=(120, 100), size=(100, 25))

        self.itemNumText = wx.StaticText(panel4, label="项目编号：", pos=(10, 140))
        self.itemNum = wx.TextCtrl(panel4, -1, value="", pos=(120, 140), size=(100, 25))
        #self.Bind(wx.EVT_TEXT, self.EvtText_doc_num, self.docNum)

        self.appSoftwareText = wx.StaticText(panel4, label="下位机软件版本：", pos=(10, 200))
        self.appSoftware = wx.TextCtrl(panel4, -1, value="V1.0.14", pos=(120, 200),size=(100, 25))
        #self.Bind(wx.EVT_TEXT, self.EvtText_app_software, self.appSoftware)

        self.terminalSoftwareText = wx.StaticText(panel4, label="维护终端软件版本：", pos=(10, 240))
        self.terminalSoftware = wx.TextCtrl(panel4, -1, value="V2.0.9", pos=(120, 240), size=(100, 25))
        #self.Bind(wx.EVT_TEXT, self.EvtText_terminal_software, self.terminalSoftware)

        #开始生成测试报告按钮
        self.start = wx.Button(panel4, -1,"开始生成报告", pos=(70, 300), size=(160, 60))
    ...
    def EvtText_change_order_path(self, event):
        dialog = wx.DirDialog(None, "请选择变更单路径", style=wx.DD_DEFAULT_STYLE | wx.DD_NEW_DIR_BUTTON)
        if dialog.ShowModal() == wx.ID_OK:
            self.change_order_path = dialog.GetPath()
            self.changeOrderPath.SetValue(self.change_order_path)
        dialog.Destroy()
    ...
    def EvtText_test_data_svn(self, event):
        ...
        self.num_ensure.Enable(True)
    def EvtText_change_order_svn(self, event):
        self.change_order_svn = event.GetString()
    ...
    def OnClick_start(self, event):
        ...
        dic = {'system_doc_path':self.system_doc_path,'system_doc_svn':self.system_doc_svn,'station_doc_path':self.station_doc_path,
                ...
                'bom_list_svn':self.bom_list_svn,'change_order_path':self.change_order_path,'change_order_svn':self.change_order_svn,
               'test_data':self.test_data,'doc_name':self.docName.GetValue(),
                'doc_num':self.docNum.GetValue(),'item_num':self.itemNum.GetValue(),
               'app_software':self.appSoftware.GetValue(),'terminal_Software':self.terminalSoftware.GetValue()}
